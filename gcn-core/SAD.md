...
│       │       ├── normalize.rs      # normalize_label() — fold accents/ligatures, dédup lexicale
...
│       │   └── src/
│       │       ├── lib.rs
│       │       ├── pearl.rs         # Pearl L1-L3 + temporel, abductif, SPOF
│       │       ├── query.rs         # GCN-QL parser + execute() — 21 types de requêtes
│       │       ├── analogy.rs       # Matching par signatures topologiques O(E)
│       │       ├── export.rs        # to_json(), to_dot()
...
pub struct CausalNode {
    ...
    pub parent: Option<NodeId>,          // Hiérarchie multi-échelle (ZOOM_IN/OUT)
}
...
pub struct CausalEdge {
    ...
    pub provenance: Option<Provenance>,  // CIR v2 — traçabilité (immutable)
    pub derivation: Option<Derivation>,  // CIR v2 — arête inférée?
}

// Traçabilité de l'extraction — 5 champs immutables par arête
pub struct Provenance {
    pub doc_ref: Option<String>,         // pointeur document source
    pub span: SourceSpan,                // TokenSpan | CodeSpan | Synthetic
    pub extraction_method: ExtractionMethod,
    pub model_version: String,
    pub extracted_at: String,            // ISO 8601 UTC
}

pub enum ExtractionMethod {
    SymbolicRust,      // frontends Rust (règles)
    MlPython,          // pipeline Python (couches ML)
    SilverAnnotator,   // annotation automatique (LLM, bootstrap)
    Derived,           // inférence (A→C déduit de A→B + B→C)
}

// Justification d'une relation inférée
...
Syntaxe GCN-QL complète (tous niveaux de Pearl + extensions) :
...
COUNTERFACTUAL <label>?         # "Si X n'avait pas eu lieu ?"

# Raisonnement temporel
CHAIN_T <a> -> <b>?             # Chaîne causale avec gaps temporels cumulés
BEFORE? <a> -> <b>              # Ordre temporel entre deux nœuds
DELAY <a> -> <b>?               # Estimation du délai causal

# Raisonnement abductif
EXPLAIN <label>?                # Hypothèses d'explication (BFS inverse, score min_conf / (1+depth))

# Méta-graphe
DENSITY [<label>]?              # Densité globale ou locale (arêtes / paires possibles)
COVERAGE <label>?               # Couverture causale autour d'un concept
RELIABILITY <label>?            # Fiabilité : confiance moyenne + ratio provenance

# Normatif
DIFF <a> -> <b>?                # Écart normatif : confiance observée vs attendue (1.0)

# Adversariel
CENTRALITY <label>?              # Centralité de degré pondérée (confiance × degré in/out)
SPOF?                           # Points de défaillance unique (nœuds dont le retrait coupe le plus de chemins)

# Multi-échelle (hiérarchie parent)
ZOOM_IN <label>?                # Enfants d'un nœud dans la hiérarchie
ZOOM_OUT <label>?               # Parent d'un nœud
AGGREGATE <label>?               # Résumé du sous-arbre (enfants, arêtes sortantes, confiance)

# Analogie
ANALOGY <a> -> <b>?             # Arêtes similaires au patron (signature topologique, O(E))
```

21 types de requêtes. Parser à la main dans `gcn-backend/src/query.rs` (pas de dépendance externe). Matching par `normalize_label()` pour la résolution des labels.
...
- **DA-9** : CIR v2 — `Provenance` est immutable (5 champs fixés à l'extraction). `Derivation` est séparée de l'origine d'ingestion. Backward compat : `#[serde(skip_serializing_if, default)]` sur les champs optionnels.
- **DA-10** : `normalize_label()` (ligatures → multi-char, fold accents FR/ES, lowercase, strip ponctuation) est le point unique de normalisation pour la dédup lexicale et le matching GCN-QL.
- **DA-11** : Guards de robustesse — SPOF N_max=500 (O(V²·(V+E)) protégé), `_safe_conf()` NaN/Inf clamp [0,1], `split_pair()` unifié pour parsing bi-argument (`->` / `,` + empty guard), `_split_two()` pour labels multi-mots Python, `_HANDLES_LOCK` thread safety sur le cache npz.
...
| Pearl+ | CIR v2 (provenance, normalize, parent), raisonnement temporel/abductif/méta/normatif/adversariel/multi-échelle/analogie — 21 types GCN-QL | ✅ Terminé |
...
8. **End-to-end** : `cargo test --workspace` passe (185 tests Rust), `cargo clippy -- -D warnings` propre, `python -m pytest gcn-python/tests/` passe (425 tests Python, 1 xfailed).
