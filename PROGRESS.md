...
v2.4.1  ████████████████████  100%  Mise à niveau infra — edge_norm, schéma v2.0, checkpoint atomique
SB-fix  ████████████████████  100%  9 patches silent bugs anti-biais/anti-crash (C2.1→C1.4)
...
## Mise à niveau v2.4.1 — Infrastructure normalisation + schéma v2.0 ✅

**Date :** 2026-09-20
**Périmètre :** 11 étapes implémentées, 3 gatées (LinkPredHead, TrainableDecoder multi-vecs, PyO3). Aucun changement Rust.

### Nouveaux modules

| Module | Fichier | Rôle |
|--------|---------|------|
| `edge_norm.py` | `data/edge_norm.py` | `normalize_node_id` (`n{int:03d}`, idempotent), `normalize_edge` (sanitization ANSI+CRLF, `relation`/`confidence`/`negated` absents → `None`+warn) |
| `graph_vecs.py` | `data/graph_vecs.py` | Cache SHA256-clé + manifest JSON, LRU mmap, hash canonique poids triés, fallback warn |
| `migrate_v1_v2.py` | `gcn-datasets/scripts/migrate_v1_v2.py` | Migration `source→sources` + `schema_version 2.0`, assert non-perte |

### Fichiers modifiés

| Composant | Modification |
|-----------|-------------|
| `data/schema.py` | `EdgeRecord.sources` canonique, shim `source` → `sources`, `from_legacy`, `confidence=None`, `negated=None` |
| `data/json_reader.py` | `sources` prioritaire sur `source` (warn conflit), `confidence`/`negated` absents → `None`+warn |
| `data/loader.py` | `TrainingSample.hyperedge_map` (default) ; arêtes N-aires gap>1 → `frozenset(sources)` dans hyperedge_map (gaté) |
| `data/verbalize_loader.py` | `subgraph`/`source_sentences`, tri gold<silver, version-check warn |
| `training/checkpoint.py` | Métadonnées arch (`d_hidden`, `vocab_size`, `bidi_flag`) ; atomique POSIX (tmp+replace) ; clés inconnues → warn+ignore ; triplet sémantique incompatible → `ValueError` |
| `training/train.py` | `copy2` best-checkpoint atomique (tmp+replace) |
| `verbalizer/instructions.py` | `add_cir` accepte dicts, `add_discourse_block`, `find_causes`/`find_effects(keyword, min_level)` déterministes, `conf None→"?"` |
| `verbalizer/query_report.py` + `decoder.py` | `max_source_chars=200` word-aware, sanitization, groupement `(n,min,max)` trié, contradictions `prevent`/`filter`+negated |
| `discuss.py` | `_split_lines(min_line_len)`, `extract_concepts_from_cir`, fallback `engine.analyze()` → retry tracé |
| `index.py` | `--min-line-len`, segmentation phrases, ids `sNNN_nMMM`, `add_discourse_block`, `except` → `log.warning` |

### Tests

- `tests/test_mise_a_niveau.py` : 8 tests — normalisation ids, sanitization, hyperedge bypass, find_match_level ordre, truncation word-aware, graph_vecs roundtrip, checkpoint atomique+arch, migrate non-perte
- **257 Python** (4 skipped stables), 144 Rust

## Phase SUP — Supervision silver FR+EN+code ✅ (clôturée 2026-09-23)

**Référence :** `DATA/supervision/verdicts_supervision.json` (+ `cloture_20260923`)

| Étape | Détail | Statut |
|-------|--------|--------|
| Silver FR v16 | 358 (200+158), UD réelle, gates negation/mention/sens-POS/titre-rubrique/nom/déontique | ✅ ACCEPTÉ aveugle 0 critique |
| Silver EN v16 | 348 (200+148), UD réelle en_core_web_sm, retypage 742 nœuds (convention FR) | ✅ ACCEPTÉ 0 critique/50 ×2 (seed 999) |
| Code/doc EN v16 | 31, lot entier vérifié | ✅ ACCEPTÉ 0 critique/31 |
| Fusion v2 | 2398 uniques (1661 + 358 + 348 + 31), split 1666/350/382 stratifié seed 42 | ✅ |
| Train model_v2 | 100ep lr 0.0005 GAT+bidi : loss 4.62→3.39, edge_acc 0.10→0.40 | ✅ |
| Éval | moi val 0.3124 / test 0.3392 / node 0.2357 vs baseline 0.1880/0.2431 (+0.124/+0.096) | ✅ trusted |
| Errata | `errata_gold.json` (2) + `errata_supervision_v2.json` (3 spans inversées s0057/s0259/s0643, tombstones quarantaine) | ✅ acté, exclusion prochain run |

**Limites actées (dette, non bloquantes) :** 7 backward légitimes non supervisables (harnais),
241 existant provisoires (réinjection UD prochain run), IDs sXXXX inter-langues (namespacing futur),
27k quarantaine LLM + gold exhaustif hors-périmètre, superviseur signaleur 0.833 permanent.
**Échelle :** chiffres NON comparables au 0.468 (BENCHMARK §9b/§10).

---

## Phase EMB — Validation embeddings ✅ (exécutée 2026-09-25)

**Référence :** `BENCHMARK.md` §11, `REMEDIATION-DIAGNOSTIC.md` §6/§8/§12

**Dataset :** `DATA/supervision/train_v2/` — 1665 train / 350 val (split stratifié seed 42).
**Backend :** GAT + bidirectionnel + weighted-loss, 20 epochs, lr=0.001, seed 42.

| Config | best val_edge_f1 | @ep | best val_node_f1 | @ep | Verdict |
|--------|-----------------|-----|------------------|-----|---------|
| Baseline (dim 0) | 0.1579 | 3 | 0.1176 | 20 | Référence |
| Emb128 apprenables | 0.2120 | 17 | **0.1519** | 20 | +34% edge ✅ |
| wiki.fr 300d gelé | 0.2027 | 14 | 0.1152 | 19 | +28% edge |
| wiki.fr 300d fine-tuné | **0.2292** | 20 | 0.1190 | 12 | **+45% edge** ✅ |

**Conclusions :**
- Les embeddings améliorent systématiquement les edges (+28% à +45% relatif).
- Fine-tuning wiki.fr 300d = meilleure config mesurée pour les edges (0.2292).
- Emb128 apprenables = meilleure config pour les nœuds (0.1519).
- Aucune config ne converge à 20 epochs (val_loss encore décroissante).
- Toutes restent loin de la cible production (> 0.40). Bloquant principal : données.

**CSVs :** `benchmark-validation/v2_baseline_20ep.csv`, `v2_emb128_20ep.csv`,
`v2_fasttext_frozen_20ep.csv`, `v2_fasttext_finetune_20ep.csv`.

**Prochaines étapes :**
1. Runs 50–100 epochs (convergence non atteinte)
2. Seed-sweep (7, 42, 123) sur configs B et D
3. Annotation 800 phrases ciblant nœuds rares (`BENCHMARK.md` §5)

---

## Phase Pearl+ — Extensions raisonnement Pearl P1–P3 ✅

**Objectif :** étendre le moteur Pearl au-delà de WHY/WHAT/CHAIN/DO/COUNTERFACTUAL avec raisonnement temporel, abductif, méta-graphe, normatif, adversariel, multi-échelle et analogique. CIR v2 : provenance, dérivation, normalisation, hiérarchie.

**Date :** 2026-09-26

### P1 — CIR v2 et fondations

| Composant | Fichier | Statut |
|---|---|---|
| `Provenance` (5 champs immutables) | `gcn-ir/src/edge.rs` | ✅ |
| `Derivation` (arêtes source) | `gcn-ir/src/edge.rs` | ✅ |
| `ExtractionMethod` (4 variantes) | `gcn-ir/src/edge.rs` | ✅ |
| `now_utc_iso8601()` (sans chrono) | `gcn-ir/src/edge.rs` | ✅ |
| `normalize_label()` (ligatures, accents, ponctuation) | `gcn-ir/src/normalize.rs` | ✅ |
| `AliasTable` (variante → canonique) | `gcn-knowledge/src/lexicon.rs` | ✅ |
| `parent: Option<NodeId>` sur `CausalNode` | `gcn-ir/src/node.rs` | ✅ |
| Diagnostic `MissingProvenance` | `gcn-middleend/src/validate.rs` | ✅ |
| Backward compat `#[serde(skip_serializing_if, default)]` | `gcn-ir/src/edge.rs`, `node.rs` | ✅ |

### P2 — Raisonnement temporel et abductif

| Composant | Fichier | Statut |
|---|---|---|
| `TemporalLink`, `chain_temporal()` | `gcn-backend/src/pearl.rs` | ✅ |
| Queries `ChainT`, `Before`, `Delay` | `gcn-backend/src/query.rs` | ✅ |
| `AbductionHypothesis`, `abduct()` (BFS inverse) | `gcn-backend/src/pearl.rs` | ✅ |
| Query `Explain` | `gcn-backend/src/query.rs` | ✅ |

### P3 — Méta, normatif, adversariel, multi-échelle, analogie

| Composant | Fichier | Statut |
|---|---|---|
| Queries `Density`, `Coverage`, `Reliability` | `gcn-backend/src/query.rs` | ✅ |
| Query `NormDiff` | `gcn-backend/src/query.rs` | ✅ |
| `count_reachable_pairs[_without]()`, `Centrality`, `Spof` | `gcn-backend/src/pearl.rs`, `query.rs` | ✅ |
| `ZoomIn`, `ZoomOut`, `Aggregate` (hiérarchie `parent`) | `gcn-backend/src/query.rs` | ✅ |
| `EdgeSignature`, `find_analogies()` (O(E), poids 0.40/0.30/0.20/0.10) | `gcn-backend/src/analogy.rs` | ✅ |

### Python — verbalizer aligné

| Composant | Fichier | Statut |
|---|---|---|
| `_normalize()` (unicodedata NFD→ASCII) | `verbalizer/instructions.py` | ✅ |
| 14 handlers (`_chain_t`, `_before`, …, `_aggregate`) | `verbalizer/instructions.py` | ✅ |
| Provenance sur chaque arête émise | `pipeline/ir_emitter.py` | ✅ |
| Fallbacks confiance corrigés → 1.0 (neutre) | `verbalizer/instructions.py` | ✅ |
| Poids analogie alignés Rust/Python | `verbalizer/instructions.py` | ✅ |

### Frontends — mise à jour constructeurs

| Composant | Fichier | Statut |
|---|---|---|
| `provenance: Some(…)`, `derivation: None` sur toutes les arêtes | `frontend-fr`, `frontend-en`, `frontend-code` | ✅ |
